...
import bcrypt from 'bcrypt'; // 비밀번호 해시를 위한 라이브러리
...
const SALT_ROUNDS = 10; // 비밀번호 해시용 salt 라운드 설정

// 회원가입을 검사하는 Joi
const 회원가입검사 = Joi.object({
  id: Joi.string().min(3).email().required().messages({
    'string.email': 'ID는 유효한 이메일 형식이어야 합니다.',
    'string.min': 'ID는 최소 3글자 이상이어야 합니다.',
...
async function signUp(req, res) {
  ...
  try {
    ...
    const newUser = await prisma.user.create({
      data: { account: id, name, password: hashedPassword },
    });
